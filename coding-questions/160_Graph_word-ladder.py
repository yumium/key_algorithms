...
class SolutionAStar:
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        gScore = {word: INF for word in wordList}
        gScore[beginWord] = 0

        fScore = {word: INF for word in wordList}
        fScore[beginWord] = self.distance(beginWord, endWord)

        openSet = [(fScore[beginWord], beginWord)]
        closeSet = set()
        position = {beginWord: 0}

        parent = {}
        print('start')

        while len(openSet) > 0:
            current = self.extract_min(openSet, position)
            closeSet.add(current)
            if current == endWord:
                return (gScore[endWord] + 1, parent)

            for i in range(len(current)):
                for c in 'abcdefghijklmnopqrstuvwxyz':
                    next_word = current[:i] + c + current[i+1:]
                    if next_word in wordList and next_word not in closeSet:
                        tentative_gScore = gScore[current] + 1
                        if tentative_gScore < gScore[next_word]:
                            parent[next_word] = current
                            gScore[next_word] = tentative_gScore
                            fScore[next_word] = tentative_gScore + self.distance(next_word, endWord)
                            openSet.append((fScore[next_word], next_word))
                            if next_word not in position:
                                self.insert(openSet, fScore[next_word], next_word, position)
                            else:
                                self.bubble_up(openSet, position[next_word], fScore[next_word], position)
        return (0, parent)

    def distance(self, this, that):
        res = 0
    ...
    def insert(self, openSet, priority, word, position):
        openSet.append((priority, word))
        position[word] = len(openSet)-1
        self.bubble_up(openSet, len(openSet)-1, priority, position)

    def bubble_up(self, openSet, i, new_priority, position):
        N = len(openSet)
        openSet[i] = (new_priority, openSet[i][1])
        while i > 0 and openSet[(i-1)//2][0] > openSet[i][0]:
            self.swap(openSet, i, (i-1)//2, position)
            i = (i-1) // 2

    def extract_min(self, openSet, position):
        if len(openSet) == 1:
            position = {}
            return openSet.pop()[1]
        
        self.swap(openSet, 0, len(openSet)-1, position)
        res = openSet.pop()[1]
        position.pop(res)
        self.min_heapify(openSet, 0, position)
        return res

    def min_heapify(self, openSet, i, position):
        iMin, N = i, len(openSet)
        if 2*i+1 < N and openSet[2*i+1][0] < openSet[i][0]:
            iMin = 2*i+1
        if 2*i+2 < N and openSet[2*i+2][0] < openSet[i][0]:
            iMin = 2*i+2
        if iMin != i:
            self.swap(openSet, i, iMin, position)
            self.min_heapify(openSet, iMin, position)

    def swap(self, openSet, i, j, position):
        i_key = openSet[i][1]
        j_key = openSet[j][1]
...
res, parent = sol.ladderLength(beginWord, endWord, wordList)
